 ...
 *
 * Idempotency: a 409 is returned synchronously if the scenario is already
 * in turn_state='advancing' when the request lands. The lock is acquired
 * via SELECT ... FOR UPDATE inside a transaction so concurrent advance
 * requests serialize cleanly.
 */

import type pg from "pg";
import { runBetweenTurnAgents } from "../stubs/agent-runtime.ts";
import { applyOverrides } from "../stubs/override-gateway.ts";
import { publishTurnState } from "../stubs/websocket-fanout.ts";
...
export async function advanceTurn(
  ...
  try {
    ...
    // Pull entities + last N events for the closing turn. Writing this in
    // a transaction so the snapshot row + the current_turn bump in step 5
    // commit atomically.
    await client.query("BEGIN");
